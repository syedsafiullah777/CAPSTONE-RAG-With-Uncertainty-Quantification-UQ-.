#!/usr/bin/env python3
"""Phase 9 smoke: Multi-Agent RAG on a small set of frozen questions."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 9 Multi-Agent RAG smoke")
    ...
    log = get_logger(run_id=run_id, phase="phase9", architecture="multi_agent")
    ...
    fp_path = out_dir / "phase9_runtime_fingerprint.json"
    ...
    for row in questions:
        ...
        if (
            ...
            or not result.retrieved_evidence
            or not verify
        ):
            failures += 1
            log.info(
                "FAIL qid=%s error=%s n_evidence=%s verify=%s answer_len=%s",
                row["id"],
                result.error,
                len(result.retrieved_evidence),
                verify.get("verification_score"),
                len(result.answer or ""),
            )
        else:
            log.info(
                "OK qid=%s n_evidence=%s verify=%.4f status=%s latency=%.2fs",
                row["id"],
                len(result.retrieved_evidence),
                float(verify.get("verification_score") or 0.0),
                verify.get("status"),
                result.latency_seconds,
            )

    raw_path = out_dir / "phase9_multi_agent_smoke.json"
    smoke_payload = {
        "phase": 9,
        "run_id": run_id,
    ...
    jsonl_path = out_dir / "phase9_multi_agent_smoke.jsonl"
    ...
    validation = ValidationRecord(
        phase=9,
        test_name="phase9_multi_agent_smoke",
        command=f"PYTHONPATH=. python scripts/smoke_multi_agent.py --backend {args.backend or 'auto'} --limit {args.limit}",
        environment=summarize_environment(fingerprint),
        expected="Each case: evidence, draft answer, verification_result, no error; decision=ANSWER",
        ...
        extra={
            ...
            "evidence_md": "project_record/evidence/phase9_validation.md",
        },
    )
    val_path = out_dir / "phase9_smoke_test.json"
