"""Phase 7 tests: fingerprint + backend factory (mock) + smoke artefacts when present."""
...
def test_phase7_smoke_artefacts_if_present() -> None:
    root = project_root()
    smoke = root / "results" / "config" / "phase7_smoke_generate.json"
    ...
    assert data["phase"] == 7
    assert data["generation"]["text"].strip()
    assert data["generation"]["backend"] in {"ollama_dev", "llama_cpp", "transformers", "mock"}
